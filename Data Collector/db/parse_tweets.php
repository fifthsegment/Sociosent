...
function checkCommand($campaign_id_parent){
	global $shouldIrun;
	global $oDB;
	  $query = 'SELECT campaign_id, command ' .
    	'FROM campaigns WHERE campaign_id = "'.$campaign_id_parent.'"';
		//$oDB = new db;
...
while (true && $shouldIrun) {
   //print $shouldIrun;

  // Process all new tweets
  $query = 'SELECT cache_id, raw_tweet, campaign_id ' .
    'FROM json_cache';
  $result = $oDB->select($query);
	checkCommand($campaign);
  while($row = mysqli_fetch_assoc($result)) {
