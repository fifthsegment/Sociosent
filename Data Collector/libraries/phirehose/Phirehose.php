...
abstract class Phirehose
{
  ...
  */
  ...
  private $shouldIrun = true;
  public $campaign_id_parent;
  public $dbhandler_parent;
  ...
  * @todo Shouldn't really hard-code URL strings in this function.
  ...
  public function checkCommand(){
	  $query = 'SELECT campaign_id, command ' .
    	'FROM campaigns WHERE campaign_id = "'.$this->campaign_id_parent.'"';
		//$oDB = new db;
  	  $result = $this->dbhandler_parent->select($query);
  ...
  /**
   * Connects to the stream API and s the stream. Each status update in the stream will cause a call to the
   * handleStatus() method.
   *
  ...
  public function consume($reconnect = TRUE)
  {
    ...
    do {
		...
		print "\n\n\n\n\nCampaign ID : ".$this->campaign_id_parent."\n\n\n";
      ...
      while ($this->shouldIrun && $this->conn !== NULL && !feof($this->conn) &&
        ...
        while(!feof($this->conn) && $this->shouldIrun){
			//print $this->shouldIrun."hello";
      ...
      // Reconnect
    } while ($this->reconnect && $this->shouldIrun);
